---
name: add-related-records
description: Show a parent record with its one-to-many child resources inline — a titled, compact related-records table (column config, "View all" link, per-row action, empty state). Use on a detail page that lists a record's children (a customer's orders + invoices). Ships a copy-ready template.
---
...
```bash
cp .claude/skills/add-related-records/templates/RelatedList.tsx src/components/data/RelatedList.tsx
# Optional: a gallery page showcasing a parent + two related lists
cp .claude/skills/add-related-records/templates/related-records.tsx src/routes/_app/<name>.tsx
```
