---
name: add-rbac
description: Add roles & permissions (authorization) on top of the template's authentication — a Role/Permission matrix, a server-side requireRole guard, and a RoleGate component for gating UI. Use when different users must see/do different things. Ships copy-ready code.
---
...
```bash
cp .claude/skills/add-rbac/templates/rbac.ts src/lib/rbac.ts
cp .claude/skills/add-rbac/templates/RoleGate.tsx src/components/RoleGate.tsx
```
